...
import {
  ...
  ArrowUpDown, Download, FileText, AlertTriangle, MessageCircle,
} from "lucide-react";
import {
...
import { AuriChat } from "./auri-chat";
    ...
    <div className="au-root">
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, color: C.dim }}>
          <button onClick={() => setChatOpen(true)} aria-label="Open Auri assistant"
            style={{
              background: "none", border: "none", color: chatOpen ? C.gold : C.dim,
              cursor: "pointer", padding: 0, display: "flex", alignItems: "center",
              transition: "color .15s ease",
            }}
            onMouseEnter={(e) => (e.currentTarget.style.color = C.gold)}
            onMouseLeave={(e) => (e.currentTarget.style.color = chatOpen ? C.gold : C.dim)}
          >
      ...
      {chatOpen && <AuriChat onClose={() => setChatOpen(false)} />}
    </div>
  );
}
